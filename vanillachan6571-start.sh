...
if [ -x "$(command -v java)" ]; then
  ...
  if [ -x "$(command -v screen)" ]; then
    screen -DmS CloudNet java -Xms256M -Xmx1024M -XX:+UseZGC -XX:+PerfDisableSharedMem -XX:+DisableExplicitGC -jar launcher.jar nogui
  elif [ -x "$(command -v tmux)" ]; then
    tmux new-session -As CloudNet java -Xms256M -Xmx1024M -XX:+UseZGC -XX:+PerfDisableSharedMem -XX:+DisableExplicitGC -jar launcher.jar nogui
  else
